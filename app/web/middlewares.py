...
from aiohttp.web_exceptions import HTTPException, HTTPUnprocessableEntity, HTTPForbidden, HTTPUnauthorized
    ...
    if request.path != "/admin.login":
        if "AIOHTTP_SESSION" not in request.cookies:
            return error_json_response(
                http_status=401, status=HTTP_ERROR_CODES[HTTPUnauthorized.status_code])
